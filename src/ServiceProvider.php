...
namespace AltDesign\SpamAddon;

use AltDesign\SpamAddon\Listeners\FormSubmittedListener;
...
class ServiceProvider extends AddonServiceProvider
{
    ...
    {
        Permission::register('view spam-addon')->label('View Spam Addon');
    }

    private function addToNav(): void
    {
        Nav::extend(function ($nav): void {
            $nav->content('Review Spam')
                ->section('Tools')
                ->can('view alt-akismet')
                ->route('spam-addon.index')
    ...
    {
        $this->publishes([
            __DIR__ . '/../config/spam-addon.php' => config_path('spam-addon.php'),
        ], 'alt-design/spam-addon');

        Statamic::afterInstalled(function ($command): void {
            $command->call('vendor:publish', ['--tag' => 'alt-design/spam-addon']);
        });
    }

    private function setupForma(): void
    {
        Forma::add('alt-design/spam-addon', ConfigController::class);
    }
}
